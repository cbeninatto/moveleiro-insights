...
# ==========================
# STREAMLIT COMPAT HELPERS (width / deprecated params)
# ==========================
...
if df_rep.empty:
    ...
    highlights_lines = [
        f"Faturamento • Melhor: {best_fat['MesLabel']} — {format_brl(best_fat['Valor'])}",
        f"Faturamento • Pior: {worst_fat['MesLabel']} — {format_brl(worst_fat['Valor'])}",
        f"Volume • Melhor: {best_vol['MesLabel']} — {format_un(best_vol['Quantidade'])}",
        f"Volume • Pior: {worst_vol['MesLabel']} — {format_un(worst_vol['Quantidade'])}",
    ]

st.markdown("---")

# =====================================================================
# MOVED UP: EVOLUÇÃO – FATURAMENTO x VOLUME (now right after Destaques)
# =====================================================================
...
if chart_curr is None:
    ...
    st_altair_stretch(chart_curr, key="evol_curr_chart")
...
if chart_prev is None:
    ...
    st_altair_stretch(chart_prev, key="evol_prev_chart")

# =====================================================================
# MOVED UP: CATEGORIAS VENDIDAS (now right after Evolução)
# =====================================================================
st.markdown("---")
...
if df_rep.empty:
    ...
    if total_cat <= 0:
        ...
        with col_pie_cat:
            ...
            st_plotly_stretch(fig_cat, key="cat_pie")
...
if df_rep.empty:
    ...
    if total_valor_all <= 0:
        ...
        with c_left:
            ...
            st_plotly_stretch(fig_states, key="states_pie")
...
if df_rep.empty or clientes_atendidos == 0:
    ...
    with col_pie:
        ...
        st_plotly_stretch(fig_clients, key="clients_pie")

    with col_tbl:
        st.caption("Resumo – clientes (mais detalhado)")

        df_tbl_clients = df_clientes_full.copy()
        df_tbl_clients["Faturamento"] = df_tbl_clients["Valor"].map(format_brl)
        df_tbl_clients["% Faturamento"] = df_tbl_clients["Share"].map(lambda x: f"{x:.1%}")
        df_tbl_clients["Volume"] = df_tbl_clients["Quantidade"].map(format_un)

        df_tbl_clients = df_tbl_clients.head(15)[["Cliente", "Cidade", "Estado", "Faturamento", "% Faturamento", "Volume"]]
...
if clientes_carteira.empty:
    ...
    with col_pie_s:
        ...
        if total_clientes == 0:
            ...
            st_altair_stretch(chart_pie, key="carteira_pie")
...
# ==========================
# PDF EXPORT (dashboard-like, charts included via PNG export)
# ==========================
...
def _kpi_card(title: str, value: str, sub: str = ""):
    ...
    value_p = Paragraph(
        ...
        ParagraphStyle("KPIValue", parent=_PDF_STYLES["Body"], fontSize=12, leading=13),
    )
...
def _df_to_rl_table(df_in: pd.DataFrame, max_rows: int):
    if df_in is None or df_in.empty:
        return Paragraph("<i>(sem dados)</i>", _PDF_STYLES["Caption"])

    df_ = df_in.head(max_rows).copy()
    data = [list(df_.columns)] + df_.astype(str).values.tolist()
...
def _build_pdf_static_map_fig(df_rep_in: pd.DataFrame):
    ...
    try:
        ...
        df_cities_local = df_rep_in.groupby(["Estado", "Cidade"], as_index=False).agg(
            Valor=("Valor", "sum"),
            Clientes=("Cliente", "nunique"),
        )
        df_cities_local["key"] = (
            df_cities_local["Estado"].astype(str).str.strip().str.upper()
            + "|"
            + df_cities_local["Cidade"].astype(str).str.strip().str.upper()
        )
        df_map_local = df_cities_local.merge(df_geo_local, on="key", how="inner")
        if df_map_local.empty:
            return None

        df_map_local["Hover"] = df_map_local.apply(
...
def build_pdf_report_dashboard_layout(
    ...
    cards = []
    for i in range(5):
        title, value = kpi_items[i] if i < len(kpi_items) else ("", "")
    ...
    # Categorias donut + table
    cat_row = Table(
        [[_rl_img(png_cat, w_cm=12.9, h_cm=7.0), _df_to_rl_table(cat_table, max_rows=14)]],
    ...
    estados_row = Table(
        [[_rl_img(png_states, w_cm=12.9, h_cm=7.2), _df_to_rl_table(estados_table, max_rows=10)]],
        colWidths=[12.9 * cm, 13.4 * cm],
    )
...
if gerar_pdf:
    ...
    # build status tables for the PDF
    ...
    try:
        pdf_bytes = build_pdf_report_dashboard_layout(
            ...
            carteira_table=status_disp if isinstance(status_disp, pd.DataFrame) else pd.DataFrame(),
